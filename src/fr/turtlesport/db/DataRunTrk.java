...
public class DataRunTrk {
   ...
   * 
   */
	public DataRunTrk(int id, Timestamp time, int longitude, int latitude,
			int heartRate, int cadence, float distance, float altitude) {
		super();
		this.id = id;
		this.time = time;
   ...
   * @param intervalle en seconde
   * @return
   */
  public static List<DataRunTrk> cloneList(List<DataRunTrk> listTrksOriginal, int interval) {
    ...
    for (int i=1 ; i <  len; i++) {
//      long time = listTrksClone.get(i).getTime().getTime() - listTrksClone.get(0).getTime().getTime();
//      if (time == interval) {
//        interval += interval;
...
////        float distDiff = trk.getDistance() - listTrksClone.get(i -1).getDistance(); 
////        long timeDiff = listTrksClone.get(i).getTime().getTime() - listTrksClone.get(i-1).getTime().getTime();    
...
//      }
		...
		if (listTrksOriginal != null && listTrksOriginal.size() > 0) {
			...
			if (!DistanceUnit.isUnitKm(DistanceUnit.getDefaultUnit())) {
				for (DataRunTrk t : listTrks) {
					t.setDistance((float) DistanceUnit.convert(
							DistanceUnit.unitKm(),
							DistanceUnit.getDefaultUnit(), t.getDistance()));
				}
			}
	...
	private static DataRunTrk cloneInvalid(DataRunTrk current, DataRunTrk trkValid,
			float distance) {
		return new DataRunTrk(current.getId(), current.getTime(),
				trkValid.getLongitude(), trkValid.getLatitude(),
				current.getHeartRate(), current.getCadence(), distance,
